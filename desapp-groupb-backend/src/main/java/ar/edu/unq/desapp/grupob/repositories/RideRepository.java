...
public class RideRepository extends GenericRepository<Ride>{
    ...
    public List<Ride> findSimilarRide(Route route){
      List<Ride> allRides = this.getHibernateTemplate().findByCriteria(DetachedCriteria.forClass(Ride.class));
      List<Ride> filteredRides = new LinkedList<Ride>();

      for(Ride ride : allRides){
          double distanceBetweenBegin = Coordinate.calculateDistanceBetween(route.getBegin(), ride.getRoute().getBegin());
          double distanceBetweenEnd = Coordinate.calculateDistanceBetween(route.getEnd(), ride.getRoute().getEnd());
